...
def correct_focus(focus_var_x, focus_var_y, x_var, y_var, ESP32, threshhold=1, weight=1):
    x_diff = np.sum(np.abs(np.abs(focus_var_x-x_var[-1])-np.abs(focus_var_x-x_var[-2:-7])))/len(x_var[-2:-7])
    y_diff = np.sum(np.abs(np.abs(focus_var_y-y_var[-1])-np.abs(focus_var_y-y_var[-2:-7])))/len(y_var[-2:-7])
